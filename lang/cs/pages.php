<?php

declare(strict_types=1);

return [

    'vizualizer' => [
        'title' => 'EDI Visualizer – mapa spojení z deníku',
        'meta' => 'Nahrajte EDI deník a získejte sdílecí mapu svých spojení – paprsky z domácího QTH, vzdálenosti, azimuty a souhrn.',
        'heading' => 'EDI Visualizer',
        'intro' => 'Nahrajte svůj EDI deník a získáte trvalý odkaz s mapou všech spojení.',
        'info_title' => 'Co se stane s vaším deníkem?',
        'info_results' => 'Deník se nezapočítává do žádných výsledků ani závodních tabulek – slouží opravdu jen k prohlížení a vykreslení mapy.',
        'info_link' => 'Dostanete soukromý odkaz s náhodným kódem. Mapu uvidí jen ten, komu odkaz sami pošlete.',
        'info_anon' => 'Soubor je na serveru uložen anonymně – jen pod tím náhodným kódem, bez vašeho jména či účtu.',
        'info_data' => 'Z deníku se přečtou jen značky a lokátory potřebné pro mapu. Nic se neukládá do databáze.',
        'upload_label' => 'EDI deník (*.edi)',
        'upload_hint' => 'Musí být ve formátu EDI (REG1TEST), max. :max kB.',
        'submit' => 'Vykreslit mapu',
        'err_required' => 'Vyberte EDI soubor.',
        'err_ext' => 'Soubor musí mít příponu .edi nebo .txt.',
        'err_max' => 'Soubor je příliš velký (max. :max kB).',
        'err_not_found' => 'Mapa nebyla nalezena – odkaz je neplatný nebo už vypršel.',
        'share' => 'Trvalý odkaz na tuto mapu:',
        'copy' => 'Kopírovat',
        'copied' => 'Zkopírováno',
        'new' => 'Nahrát další deník',
        'sum_call' => 'Volačka',
        'sum_loc' => 'Lokátor',
        'sum_band' => 'Pásmo',
        'sum_qso' => 'Spojení (QSO)',
        'sum_avg' => 'Průměrná vzdálenost',
        'sum_max' => 'Nejdelší spojení (ODX)',
        'sum_loc_unique' => 'Unikátních lokátorů',
        'sum_sq_unique' => 'Unikátních velkých čtverců',
        // Popisky vykreslované v JS (vizualizer.js)
        'js' => [
            'pts' => 'b.',
            'azimuth' => 'azimut',
            'home' => 'domácí QTH',
        ],
    ],

    'generator' => [
        'title' => 'Generátor EDI deníku – ruční vytvoření deníku',
        'meta' => 'Vytvořte si EDI deník (REG1TEST) ručně – zapište hlavičku a spojení a v reálném čase vidíte složený .edi text, body, násobiče i mapu spojení.',
        'heading' => 'Generátor EDI deníku',
        'intro' => 'Nemáte deníkový program? Zapište hlavičku a jednotlivá spojení ručně. Vpravo se v reálném čase skládá hotový .edi soubor, počítá skóre a kreslí mapa. Hotový deník můžete stáhnout, nebo na 144 MHz rovnou podat jako hlášení.',
        'header' => 'Hlavička deníku',
        'contact' => 'Kontakt',
        'qsos' => 'Spojení (QSO)',
        'pcall' => 'Volací značka (PCall)',
        'pwwlo' => 'Lokátor (PWWLo)',
        'tdate' => 'Datum závodu',
        'pband' => 'Pásmo',
        'psect' => 'Sekce',
        'single' => 'SINGLE (jeden operátor)',
        'multi' => 'MULTI (více operátorů)',
        'spowe' => 'Výkon [W]',
        'rname' => 'Jméno',
        'rhbbs' => 'E-mail',
        'rphon' => 'Telefon',
        'stxeq' => 'TX/RX zařízení',
        'sante' => 'Anténa',
        'remarks' => 'Poznámka (Remarks)',
        'add_qso' => 'Přidat spojení',
        'remove_qso' => 'Odebrat spojení',
        'col_time' => 'Čas',
        'col_call' => 'Volačka',
        'col_mode' => 'Provoz',
        'col_rst_s' => 'RST odesl.',
        'col_rst_r' => 'RST přij.',
        'col_wwl' => 'Lokátor',
        'qso_hint' => 'Čas zadávejte ve formátu HHMM (UTC), např. 0815. Pořadová čísla se přidělují automaticky. Započítávají se jen spojení v závodním okně 08:00–11:00 UTC.',
        'stat_qso' => 'Spojení',
        'stat_mult' => 'Násobiče',
        'stat_points' => 'Body',
        'score_hint' => 'Orientační skóre dle pravidel VKV PA (body za spojení z lokátorů × počet velkých čtverců včetně vlastního).',
        'map' => 'Mapa spojení',
        'edi_text' => 'Náhled EDI souboru',
        'download' => 'Stáhnout .edi',
        'submit' => 'Podat jako hlášení',
        'submit_hint' => 'Deník se uloží do aktuálního průběžného kola (jako při nahrání EDI). Vyplňte jméno, e-mail a telefon.',
        'submit_only_144' => 'Rovnou podat jako hlášení lze jen deník na pásmu 144 MHz. Pro ostatní pásma deník stáhněte a odešlete přes „Odeslat deník".',
    ],

    'home' => [
        'title' => 'Výsledky a online hlášení VKV závodů',
        'heading' => 'VKV Provozní aktiv',
        'subtitle' => 'Elektronické hlášení výsledků závodů VKV PA',
        'meta' => 'VKV Provozní aktiv – online hlášení výsledků amatérských VKV závodů: nahrávání EDI deníků, automatické bodování, průběžné a roční výsledkové listiny.',
        'state_upcoming' => 'Nadcházející kolo',
        'state_running' => 'Závod právě probíhá',
        'state_deadline' => 'Probíhá příjem hlášení',
        'state_evaluating' => 'Zpracování výsledků',
        'state_evaluated' => 'Výsledky jsou k dispozici',
        'contest_date' => 'Datum závodu',
        'deadline' => 'Uzávěrka',
        'evaluated_at' => 'Vyhodnoceno',
        'evaluating_note' => 'Výsledky jsou zpracovávány – tabulka se automaticky obnovuje.',
        'countdown_to_start' => 'do začátku závodu',
        'countdown_to_end' => 'do konce závodu',
        'countdown_to_deadline' => 'do uzávěrky hlášení',
        'btn_submit' => 'Odeslat deník',
        'btn_interim' => 'Průběžné výsledky',
        'btn_results' => 'Výsledková listina',
        'btn_discussion' => 'Diskuse ke kolu',
        'received_count' => '{0} Zatím nebylo přijato žádné hlášení — buďte první!|{1} Zatím přijato 1 hlášení|[2,4] Zatím přijata :count hlášení|[5,*] Zatím přijato :count hlášení',
        'last_evaluated_heading' => 'Poslední vyhodnocené kolo',
        'discussion_heading' => 'Z diskuse',
        'discussion_all' => 'celá diskuse →',
        'discussion_photos' => '{1} :count fotka|[2,4] :count fotky|[5,*] :count fotek',
        'local_time_suffix' => 'místního času',
        'live_heading' => 'Průběžné výsledky',
        'live_refresh_in' => 'Aktualizace za',
        'live_no_entries' => 'Zatím nebyla přijata žádná hlášení.',
        'quick_links' => 'Výsledky',
        'section_edi' => 'EDI',
        'section_stats' => 'Statistiky',
        'ql_submit' => 'Odeslat deník',
        'ql_submit_desc' => 'Nahrát EDI soubor',
        'ql_generator' => 'Generátor deníku',
        'ql_generator_desc' => 'Vytvořit EDI deník ručně',
        'ql_viz' => 'EDI Visualizer',
        'ql_viz_desc' => 'Mapa spojení z deníku',
        'ql_interim' => 'Průběžné výsledky',
        'ql_interim_desc' => 'Aktuální pořadí v kole',
        'ql_results' => 'Výsledková listina',
        'ql_results_desc' => 'Uzavřená kola',
        'ql_yearly' => 'Roční výsledky',
        'ql_yearly_desc' => 'Přehled za rok',
        'ql_stats' => 'Statistiky kol',
        'ql_stats_desc' => 'Mapy a rekordy kol',
        'ql_trends' => 'Dlouhodobé trendy',
        'ql_trends_desc' => 'Vývoj pásem v čase',

        // Upload window info strip
        'upload_window_label' => 'Upload okno',
        'upload_window_upcoming' => '3. neděle :date 08:00 UTC – pátek :deadline 23:59 UTC',
        'upload_window_open' => 'Otevřeno do :deadline 23:59 UTC — EDI/EDIR omezeny (anti-opisování)',
        'upload_window_closed_auth' => 'Uzavřeno — EDI dostupné po přihlášení',
        'upload_window_evaluated' => 'Vyhodnoceno — EDI dostupné po přihlášení',

        'upcoming_heading' => 'Plánovaná kola',
    ],

    'kola' => [
        'title' => 'Kola závodu – VKV PA',
        'heading' => 'Kola závodu',
        'col_date' => 'Datum konání',
        'col_deadline' => 'Uzávěrka',
        'col_name' => 'Název',
        'col_count' => 'Záznamů',
        'col_state' => 'Stav',
        'col_evaluated' => 'Vyhodnoceno',
        'col_actions' => 'Akce',
        'btn_create' => '+ Přidat kolo',
        'btn_edit' => 'upravit',
    ],

    'hlaseni' => [
        'title' => 'Hlášení – VKV PA',
        'heading' => 'Odeslání deníku a hlášení výsledků',
        'meta' => 'Odešlete svůj deník do závodu VKV PA – nahrajte EDI soubor nebo vyplňte hlášení ručně. Body se spočítají automaticky.',
        'tab_edi' => 'Import EDI',
        'tab_manual' => 'Ruční vložení',
        'heading_edi' => 'Načíst EDI soubor',
        'heading_manual' => 'Nemám EDI soubor (vyplním hlášení ručně)',
        'error_line' => 'Chybný řádek',
        'import_warnings' => 'Deník byl načten, ale upozorňujeme na:',
        'btn_upload' => 'nahrát',
        'edi_info' => 'Lze použít jakýkoli logovací software, který umí edi export a nezáleží na tom, jestli samotný deník umí počítat body pro PA, nebo ne. Zcela vyhoví standardní konfigurace pro závody, ve kterých je jeden bod za kilometr, robot si spočítá body dle pravidel PA i vyhledá v deníku násobiče.',
        'no_edi_link' => 'Nemám EDI soubor (vyplním hlášení ručně)',
        'field_period' => 'Kolo',
        'field_category' => 'Kategorie',
        'field_callsign' => 'Volací znak',
        'field_locator' => 'Lokátor',
        'field_qrp' => 'QRP (zaškrtněte, pokud jste v závodě použili výkon QRP)',
        'field_lp' => 'LP (zaškrtněte, pokud jste v závodě použili výkon do 100 W)',
        'field_qso' => 'Počet QSO',
        'field_qso_pts' => 'Bodů za QSO',
        'field_mult' => 'Násobiče',
        'field_total' => 'Celkem bodů',
        'field_name' => 'Jméno',
        'field_contact' => 'Email',
        'field_phone' => 'Telefon',
        'field_note' => 'Poznámka',
        'field_soapbox' => 'Soapbox',
        'select_period' => '--- vyberte kolo ---',
        'select_category' => '--- vyberte kategorii ---',
        'btn_clear' => 'vymazat formulář',
        'btn_send' => 'Odeslat',
        'col_pos' => 'Poř.',
        'col_callsign' => 'Značka',
        'col_locator' => 'Lokátor',
        'col_qso' => 'QSO',
        'col_mult' => 'Násobiče',
        'col_total' => 'Celkem bodů',
        'col_name_note' => 'Jméno / Poznámka',
        'col_status' => 'Stav',
        'status_ok' => 'OK',
        'status_pending' => 'Čeká',
        'interim_results' => 'Průběžné výsledky kola',
        // Náhled importu (Livewire Prihlaska)
        'processing' => 'Zpracovávám deník…',
        'review_heading' => 'Zkontrolujte načtený deník',
        'analysis_summary' => 'Analýza spojení – co se započítalo a proč',
        'edi_file_summary' => 'EDI soubor',
        'edir_dropped_hint' => '(červeně řádky, které EDIR ořízne – mimo okno)',
        'edir_summary' => 'EDIR – oříznutý na závodní okno (08:00–11:00 UTC)',
        // Editace existujícího záznamu (admin / vlastník)
        'admin_warnings' => 'Varování administrátora',
        'edit_heading' => 'Editace záznamu',
    ],

    'pribezne' => [
        'title' => 'Průběžné výsledky – VKV PA',
        'meta' => 'Průběžné výsledky aktuálního kola VKV Provozního aktivu – pořadí stanic v reálném čase během příjmu hlášení.',
        'heading' => 'Průběžné výsledky kola',
        'filter_round' => 'Kolo',
        'filter_category' => 'Kategorie',
        'filter_all' => 'Vše',
        'btn_show' => 'Zobrazit',
        'no_round' => 'Vyhodnocování právě neprobíhá – žádné kolo aktuálně nepřijímá hlášení.',
        'no_results' => 'Pro toto kolo zatím nejsou žádná hlášení.',
    ],

    'vysledky' => [
        'title' => 'Výsledková listina – VKV PA',
        'meta' => 'Výsledkové listiny jednotlivých kol VKV Provozního aktivu – pořadí v kategoriích, body, násobiče a QSO, s odkazy na deníky a statistiky.',
        'heading' => 'Výsledková listina',
        'filter_round' => 'Kolo',
        'filter_band' => 'Pásmo',
        'filter_section' => 'Sekce',
        'filter_variant' => 'Varianta',
        'filter_all' => 'Vše',
        'filter_search' => 'Hledat',
        'filter_qrp' => 'jen QRP',
        'filter_lp' => 'jen LP',
        'variant_domestic' => 'OK/OL',
        'filter_prevzeti' => 'Stav převzetí',
        'prevzeti_all' => 'Vše',
        'prevzeti_yes' => 'Jen převzaté',
        'prevzeti_no' => 'Jen nepřevzaté',
        'btn_show' => 'Zobrazit',
        'no_closed_round' => 'Žádné uzavřené kolo k zobrazení.',
        'too_many' => 'Výsledků je příliš mnoho – zobrazeno je jen prvních :count záznamů. Použijte filtr pro upřesnění.',
        'no_search' => 'Hledání „:query" v tomto kole nevrátilo žádné výsledky.',
        'no_results' => 'Pro toto kolo zatím nejsou žádné výsledky.',
        'col_pos' => 'Poř.',
        'col_callsign' => 'Značka / Jméno',
        'col_locator' => 'Locator',
        'col_qso' => 'QSO',
        'col_mult' => 'Nás.',
        'col_total' => 'Body',
        'col_soapbox' => 'Soapbox / Poznámka',
        'col_actions' => 'Akce',
        'col_edi' => 'EDI',
        'col_stats' => 'Statistiky',
        'link_contact' => 'Kontaktní údaje závodníka (admin)',
        'delete_title' => 'Smazat záznam',
        'delete_confirm' => 'Opravdu smazat záznam :callsign?',
        'btn_cancel' => 'Zrušit',
        'btn_delete' => 'Smazat',
        'skokan_title' => 'Největší skokan v kategorii (oproti poslednímu startu)',
        'vs_last_start' => 'oproti poslednímu startu',
        'same_as_last' => 'stejně jako posledně',
    ],

    'rocni' => [
        'title' => 'Roční výsledky – VKV PA',
        'meta' => 'Roční souhrnné výsledky VKV Provozního aktivu – celkové pořadí stanic za rok napříč všemi koly.',
        'heading' => 'Roční výsledky',
        'filter_year' => 'Rok',
        'filter_band' => 'Pásmo',
        'filter_category' => 'Kategorie',
        'filter_all' => 'Vše',
        'filter_qrp' => 'jen QRP',
        'filter_lp' => 'jen LP',
        'btn_show' => 'Vypsat',
        'subheading' => 'Výsledková listina za rok :year',
        'col_pos' => 'Pořadí',
        'col_callsign' => 'Značka',
        'col_total' => 'Celkem',
        'month_title' => ':round: :qso_points b. za QSO × :mult = :points b. (:qso QSO)',
        'no_results' => 'Pro tento rok nejsou žádné vyhodnocené výsledky.',
        'legend_label' => 'Výkon v měsíci:',
        'legend_qrp' => 'QRP (≤ 5 W)',
        'legend_lp' => 'LP (< 100 W)',
        'month_link_hint' => 'Klikatelné body otevřou log nebo kolo.',
    ],

    'diskuse' => [
        'title' => 'Diskuse – :round – VKV PA',
        'meta' => 'Diskuse k závodnímu kolu :round – komentáře, zážitky a fotografie účastníků VKV Provozního aktivu.',
        'heading' => 'Diskuse',
        'filter_round' => 'Kolo',
        'btn_go' => 'Přejít',
        'no_posts' => 'Pro toto kolo zatím nejsou žádné příspěvky. Buďte první!',
        'field_callsign' => 'Značka',
        'field_name' => 'Jméno',
        'field_text' => 'Text',
        'field_photo' => 'Fotografie',
        'photo_hint' => 'Můžete přidat až 5 fotek najednou (i přímo z mobilu). Obrázky se automaticky zmenší.',
        'photo_open' => 'Zvětšit fotografii',
        'photo_close' => 'Zavřít',
        'photo_prev' => 'Předchozí fotografie',
        'photo_next' => 'Další fotografie',
        'photo_too_many' => 'Najednou lze nahrát nejvýše :max fotek.',
        'ph_callsign' => 'OK1XYZ',
        'ph_name' => 'Volitelně',
        'ph_text' => 'Váš komentář nebo zážitek ze závodu…',
        'btn_submit' => 'Odeslat příspěvek',
        'btn_delete' => 'Smazat',
        'add_post' => 'Přidat příspěvek',
        'delete_title' => 'Smazat příspěvek',
        'delete_confirm' => 'Opravdu smazat příspěvek od :callsign?',
        'btn_cancel' => 'Zrušit',
        'post_count_1' => 'příspěvek',
        'post_count_few' => 'příspěvky',
        'post_count_many' => 'příspěvků',
    ],

    'viz' => [
        'title' => 'Vizualizace – :call – VKV PA',
        'meta' => 'Vizualizace deníku stanice :call – mapa spojení, grafy a statistiky závodu VKV Provozní aktiv.',
        'heading' => 'Vizualizace deníku :call',
        'subtitle_charts' => 'mapa a grafy (Leaflet + Chart.js)',
        'compare_link' => '⚔️ Porovnání deníků',
        'round_pending' => 'ℹ️ Po vyhodnocení kola budou mapy obsahovat více dat – do vrstvy CRK přibudou všechny stanice z kola a na stránce Porovnání deníků půjde srovnat deníky účastníků.',

        'stat_qso' => 'Počet QSO',
        'stat_qso_hint' => 'QSO v závodním okně s platnými souřadnicemi – jen ta vstupují do mapy a grafů.',
        'stat_unique' => 'Unique lokátory',
        'stat_unique_hint' => 'Počet různých velkých čtverců (první 4 znaky lokátoru) mezi protistanicemi.',
        'stat_maxdist' => 'Max. vzdálenost',
        'stat_maxdist_hint' => 'Nejdelší spojení deníku (ODX); vzdálenost spočítaná z lokátorů.',
        'stat_avgdist' => 'Průměr vzdálenost',
        'stat_avgdist_hint' => 'Průměrná vzdálenost přes všechna QSO se spočítanou vzdáleností.',

        'station_power' => 'Výkon',
        'station_antenna' => 'Anténa',
        'station_trx' => 'TRX',
        'station_operator' => 'Operátor',
        'station_empty' => 'nevyplněno',
        'station_soapbox' => 'Soapbox',
        'station_note' => 'Poznámka',

        'tempo_peak_hint' => 'Nejvíce QSO v klouzavém 60minutovém okně.',
        'tempo_qso_per_hour' => 'QSO/hod',
        'tempo_peak' => 'Špička :when',
        'tempo_pause_hint' => 'Nejdelší mezera mezi dvěma po sobě jdoucími QSO v závodním okně.',
        'tempo_min' => 'min',
        'tempo_longest_pause' => 'Nejdelší pauza :when',
        'tempo_avg_hint' => 'Průměrný počet QSO za hodinu přes celé závodní okno.',
        'tempo_avg' => 'Průměrné tempo',
        'tempo_uncounted_hint' => 'QSO mimo závodní okno či den závodu a QSO označená v deníku jako duplicitní (D).',
        'tempo_uncounted' => 'Nezapočítaná / označená QSO',

        'mode_heading' => 'Souhrn po druzích provozu',
        'mode_other' => 'Ostatní druhy provozu',
        'mode_pts_per_qso' => 'b. za spojení',

        'map' => 'Mapa',
        'layer_playback' => 'Přehrávání',
        'layer_crk' => 'CRK',
        'layer_jezek' => 'Ježek',
        'layer_spendliky' => 'Špendlíky',
        'layer_lokatory' => 'Lokátory',
        'layer_ctverce' => 'Obsazené čtverce',
        'engine_toggle' => 'Vykreslování mapy',
        'engine_leaflet_title' => 'Klasická mapa (Leaflet, rastrové dlaždice)',
        'engine_maplibre_title' => 'Pokročilá GPU mapa (MapLibre GL + deck.gl, vektorové dlaždice)',
        'engine_fallback' => 'Prohlížeč nepodporuje WebGL, použita klasická mapa (Leaflet).',
        'mode_filter' => 'Provoz:',
        'mode_other_short' => 'Ostatní',
        'play' => '▶ Přehrát',
        'qso' => 'QSO',
        'score_title' => 'Průběžné skóre k času na slideru: body za spojení × násobiče',

        'prubeh_caption' => 'Orientační průběh: kumulativní body za spojení × průběžný počet násobičů (vlastní čtverec :sq se počítá od začátku). Počítá se jen z QSO s platným lokátorem.',
        'az_weight_by' => 'Vážit podle:',
        'az_count' => 'Počet QSO',
        'az_km' => 'Kilometry',
        'az_points' => 'Body',
        'sezona_caption' => 'Body a pořadí stanice :call v kolech roku (z veřejné výsledkové listiny).',
        'sezona_unavailable' => 'Celoroční trend zatím není k dispozici – deník nemá přiřazené kolo nebo stanice nemá schválené záznamy.',
        'chart_png_title' => 'Stáhnout graf jako PNG',
        'country_other' => 'Ostatní',

        'uncounted_heading' => 'Nezapočítaná / označená QSO (:count) – zobrazit výpis',
        'uncounted_truncated' => 'Zobrazeno prvních :shown řádků.',
        'uncounted_reason_okno' => 'mimo závodní okno',
        'uncounted_reason_den' => 'jiný den než závod',
        'uncounted_reason_duplicita' => 'v deníku označeno jako duplicita (D)',
        'col_reason' => 'Důvod',

        'odx_heading' => 'TOP ODX – nejvzdálenější spojení',
        'odx_empty' => 'Žádná spojení se spočítanou vzdáleností.',
        'odx_hint' => 'Klik na řádek zobrazí spojení na mapě.',
        'odx_show_on_map' => 'Zobrazit na mapě',
        'col_callsign' => 'Značka',
        'col_locator' => 'Lokátor',
        'col_time' => 'Čas',
        'col_mode' => 'Mód',
        'col_points' => 'Body',
        'col_azimuth' => 'Azimut',

        // Popisky vykreslované v JS (vizualizace.js) – předávané přes window.__vizConfig.t
        'js' => [
            'pts' => 'b.',
            'mult' => 'nás.',
            'azimuth' => 'azimut',
            'new_mult' => 'nový násobič',
            'stations' => 'protistanic',
            'legend' => 'Legenda',
            'other' => 'Ostatní',
            'play' => '▶ Přehrát',
            'pause' => '⏸ Pauza',
            'score_pts' => 'b.',
            'score_mult' => 'nás.',
            'title_prubeh' => 'Průběh skóre (body za spojení × násobiče)',
            'ds_new_mult' => 'QSO s novým násobičem',
            'ds_other_qso' => 'Ostatní QSO',
            'title_timeline' => 'QSO v čase a nové násobiče (15min intervaly)',
            'az_count' => 'Směry QSO (počet)',
            'az_km' => 'Směry QSO (součet km)',
            'az_points' => 'Směry QSO (součet bodů)',
            'ds_points' => 'Body',
            'title_squares' => 'Body za spojení podle velkých čtverců',
            'qso_suffix' => 'QSO',
            'ds_rank' => 'Pořadí v kategorii',
            'title_season' => 'Sezóna :call po kolech',
            'axis_points' => 'Body',
            'axis_rank' => 'Pořadí',
            'ds_other' => 'Ostatní',
            'title_dist' => 'Rozložení vzdáleností QSO podle druhu provozu',
            'title_country' => 'QSO podle zemí (DXCC)',
            'title_prefix' => 'QSO podle prefixů',
            // Popup velkého čtverce (vrstvy Lokátory / Obsazené čtverce)
            'sq_stations' => 'stanic',
            'sq_modes' => 'Druhy provozu',
            'sq_odx' => 'ODX',
            'sq_calls' => 'Stanice',
        ],
    ],

    'porovnani' => [
        'title' => 'Porovnání deníků – :call – VKV PA',
        'meta' => 'Porovnání deníku stanice :call se soupeři z téhož kola VKV PA – mapa rozdílů v protistanicích a srovnání průběhu skóre.',
        'heading' => '⚔️ Porovnání deníků – :call',
        'subtitle' => 'porovnání s deníkem soupeře z téhož kola a kategorie',
        'viz_link' => 'vizualizace',
        'round_pending' => 'ℹ️ Porovnání deníků bude dostupné po uzávěrce, resp. vyhodnocení kola – do té doby se deníky soupeřů nezveřejňují.',
        'no_rivals' => 'V téže kategorii tohoto kola není žádný další deník, se kterým by šlo porovnávat.',
        'compare_with' => 'Porovnat :call s:',
        'pick_placeholder' => '— vyberte soupeře —',
        'pick_hint' => 'Vyberte soupeře – zobrazí se mapa rozdílů v protistanicích a porovnání průběhu skóre.',
        'sum_qso' => 'QSO',
        'sum_mult' => 'násobičů',
        'sum_points' => 'bodů',
        'only' => 'Jen :call',
        'both' => 'Udělali oba',
        'map_heading' => 'Mapa protistanic: :a vs. :b',
        'map_caption' => 'Zelené body udělal jen :a, červené jen :b, šedé oba. Vzájemné spojení obou stanic se nepočítá.',
        'prubeh_caption' => 'Orientační průběh: kumulativní body za spojení × průběžný počet násobičů (vlastní čtverec se počítá od začátku). Počítá se jen z QSO s platným lokátorem.',
        // Popisky vykreslované v JS (porovnani.js)
        'js' => [
            'both' => 'udělali oba',
            'only' => 'jen',
            'legend_both' => 'oba',
            'title_score' => 'Průběh skóre (body za spojení × násobiče)',
            'title_timeline' => 'QSO v čase (15min intervaly)',
            'title_az' => 'Směry QSO (počet)',
        ],
    ],

    'stat' => [
        'index_title' => 'Statistiky kol VKV PA',
        'index_meta' => 'Souhrnné statistiky, mapy a žebříčky jednotlivých vyhodnocených kol VKV Provozního aktivu.',
        'index_heading' => 'Statistiky kol',
        'index_subtitle' => 'Souhrny, mapy a rekordy jednotlivých vyhodnocených kol.',
        'empty' => 'Zatím není vyhodnoceno žádné kolo.',
        'card_participants' => '{0} žádní účastníci|{1} :count účastník|[2,4] :count účastníci|[5,*] :count účastníků',
        'card_entries' => '{0} žádné záznamy|{1} :count záznam|[2,4] :count záznamy|[5,*] :count záznamů',
        'card_participants_label' => 'Účastníci',
        'card_entries_label' => 'Záznamy',
        'archive_heading' => 'Archiv kol podle roku',
        'year_nav' => 'Rychlý výběr roku',
        'year_rounds' => '{1} :count kolo|[2,4] :count kola|[5,*] :count kol',
        'year_avg_participants' => 'průměr účasti :count',
        'year_entries' => '{0} žádné záznamy|{1} :count záznam|[2,4] :count záznamy|[5,*] :count záznamů',
        'best_attendance_year' => 'nejvyšší účast roku',

        'kolo_title' => 'Statistiky kola :kolo',
        'kolo_meta' => 'Souhrn, mapa stanic a obsazených čtverců a žebříčky kola :kolo VKV PA.',
        'kolo_heading' => 'Statistiky kola :kolo',

        'stat_stations' => 'Stanic',
        'stat_entries' => 'Záznamů',
        'stat_qso' => 'QSO celkem',
        'stat_points' => 'Bodů celkem',
        'stat_squares' => 'Velkých čtverců',
        'stat_odx' => 'ODX kola',

        'map' => 'Mapa',
        'layer_stations' => 'Stanice kola',
        'layer_squares' => 'Obsazené čtverce',

        'odx_heading' => 'Nejdelší spojení kola (ODX)',
        'odx_line' => ':home (:homeloc) → :call (:wwl) · :dist km',

        'top_heading' => 'TOP žebříčky kola',
        'band_all' => 'Vše',
        'top_points' => 'TOP body',
        'top_qso' => 'TOP počet QSO',
        'top_mult' => 'TOP násobiče',
        'unit_points' => 'b.',
        'unit_mult' => 'násob.',

        'layer_participants' => 'Účastníci',
        'mode_ssb' => 'SSB',
        'mode_cw' => 'CW',
        'country_other' => 'Ostatní',
        'js_stations' => 'Stanice',

        'chart_timeline' => 'Aktivita v čase (počet QSO)',
        'chart_mody' => 'Druhy provozu',
        'chart_kategorie' => 'Kategorie',
        'chart_zeme' => 'Země (TOP)',
        'chart_prefix' => 'Prefixy (TOP)',
        'chart_trend' => 'Trend posledních kol',
        'chart_pasma' => 'Podíl pásem v čase',
        'pasma_year' => '{1} :count rok|[2,4] :count roky|[5,*] :count let',
        'pasma_all' => 'Vše',

        'badge_ucast' => 'Rekord účasti!',
        'badge_skore' => 'Rekord skóre!',
        'badge_qso' => 'Rekord QSO!',
        'badge_multiplier' => 'Rekord násobičů!',

        'fact_active_square' => 'Nejaktivnější čtverec: :square (:count QSO)',
        'fact_winner_margin' => 'Náskok vítěze :call: :margin b. před druhým',
        'fact_debutants' => 'Nováčků v kole: :count',

        'hall_heading' => 'Historické rekordy',
        'hall_note' => 'Rekordy napříč všemi vyhodnocenými koly.',
        'rec_odx' => 'Nejdelší spojení (ODX)',
        'rec_ucast' => 'Rekordní účast',
        'rec_skore' => 'Nejvyšší skóre',
        'rec_qso' => 'Nejvíc QSO',
        'rec_multiplier' => 'Nejvíc násobičů',
        'rec_in_round' => 'kolo :kolo',
        'rec_stations_unit' => 'stanic',

        'layer_tok' => 'Tok mezi čtverci',

        'stanice_title' => 'Profil stanice :call',
        'stanice_meta' => 'Historie stanice :call ve VKV PA: účast v :kola kolech, body a pořadí.',
        'stanice_subtitle' => 'Historie a výsledky napříč vyhodnocenými koly.',
        's_rounds' => 'Kol',
        's_points' => 'Body celkem',
        's_qso' => 'QSO celkem',
        's_best' => 'Nejlepší pořadí',
        's_topscore' => 'Nejvyšší skóre',
        's_trend' => 'Trend bodů',
        's_history' => 'Historie účastí',
        's_log' => 'deník →',
        'col_round' => 'Kolo',
        'col_category' => 'Kategorie',
        'col_mult' => 'Násob.',
        'col_points' => 'Body',
        'col_place' => 'Pořadí',
    ],

    'trendy' => [
        'title' => 'Dlouhodobé trendy – VKV PA',
        'meta' => 'Dlouhodobé trendy VKV Provozního aktivu: jak se v čase mění podíl jednotlivých pásem mezi stanicemi napříč všemi vyhodnocenými koly.',
        'heading' => 'Dlouhodobé trendy',
        'subtitle' => 'Jak se v čase mění obliba pásem napříč všemi vyhodnocenými koly.',
        'scope_label' => 'Filtr stanic',
        'years_label' => 'Období',
        'scope_all' => 'Vše',
        'scope_domestic' => 'Domestic',
        'scope_dx' => 'DX',
        'scope_empty' => 'Pro vybraný filtr nejsou v tomto rozsahu žádná data.',
        'pasma_note' => 'Podíl podle počtu různých stanic na pásmu; základ 100 % je součet stanic přes pásma (jedna stanice může jet víc pásem). Pásmo se bere z kategorie záznamu.',
        'empty' => 'Zatím nejsou data – žádné vyhodnocené kolo.',
    ],

];
