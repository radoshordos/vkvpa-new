...
app/
├── Enums/
│   ├── MapMode.php         # Enum: jezek / spendliky / lokatory
...
└── views/
    ...
    ├── pages/              # Stránky (hlaseni, kola, vysledky, diskuse, map, edi-upload, admin/*)
...
| `MapMode` | `Jezek`, `Spendliky`, `Lokatory` | Typ mapového pohledu; hodnota (`value`) je segment URL |
...
Tři Leaflet-based mapové pohledy pro každý `Edihead` (`MapMode` enum):
