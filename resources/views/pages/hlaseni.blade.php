...
<form action="{{ route('hlaseni.store') }}" method="post" class="card mb-6">
    ...
    <input type="hidden" name="edihead_id" value="{{ (int) $val('edihead_id', $e->edi_head_id ?? 0, 0) }}">
        ...
        <x-field name="kolo" :label="__('pages.hlaseni.field_period')" required>
            <x-slot:control>
                <select id="f-kolo" name="kolo" @class(['select', 'input-err' => $errors->has('kolo')])>
                    ...
                    @foreach ($kola as $k)
                        <option value="{{ $k->id }}" @selected((int) $val('kolo', $e->round_id ?? 0) === $k->id)>{{ $k->name }}</option>
                    @endforeach
                </select>
            </x-slot:control>
        </x-field>
            ...
            <x-slot:control>
                <select id="f-kategorie" name="kategorie" @class(['select', 'input-err' => $errors->has('kategorie')])>
                    ...
                    @foreach ($kategorie as $cat)
                        <option value="{{ $cat->id }}" @selected((int) $val('kategorie', $e->category_id ?? 0) === $cat->id)>{{ $cat->name }}</option>
                    @endforeach
                </select>
            </x-slot:control>
        </x-field>
    ...
    <div class="grid grid-cols-2 gap-x-5 sm:grid-cols-4">
        <x-field name="pocet" :label="__('pages.hlaseni.field_qso')" :value="(int) $val('pocet', $e->qso_count ?? 0, 0)" required class="num" />
        <x-field name="qso_points" :label="__('pages.hlaseni.field_qso_pts')" :value="(int) $val('qso_points', $e->qso_points ?? 0, 0)" class="num" />
        <x-field name="multiplier" :label="__('pages.hlaseni.field_mult')" :value="(int) $val('multiplier', $e->multiplier ?? 0, 0)" required class="num" />
        <x-field name="body" :label="__('pages.hlaseni.field_total')" :value="(int) $val('body', $e->points ?? 0, 0)" required class="num font-bold" />
    </div>

    <div class="grid gap-x-5 sm:grid-cols-2">
        <x-field name="jmeno" :label="__('pages.hlaseni.field_name')" :value="$val('jmeno', $e->name ?? '')" required />
        {{-- Stačí vyplnit jeden kontakt (telefon NEBO e-mail) – „alespoň jeden"
             hlídá jednotně server, proto tu žádné z polí není jednotlivě
             required (platí pro EDI i ruční podání). --}}
        <x-field name="email" :label="__('pages.hlaseni.field_contact')" :value="$val('email', $e->email ?? '')" />
        <x-field name="telefon" :label="__('pages.hlaseni.field_phone')" :value="$val('telefon', $e->phone ?? '')" />
    </div>

    <x-field name="poznamka" :label="__('pages.hlaseni.field_note')">
        <x-slot:control>
            <textarea id="f-poznamka" name="poznamka" class="textarea" rows="2">{{ $val('poznamka', $e->note ?? '') }}</textarea>
        </x-slot:control>
    </x-field>
...
@foreach ($vysledky->groupBy('category_id') as $katId => $radky)
    <div class="section-head">{{ __('pages.hlaseni.interim_results') }} — {{ $katMap[$katId]->name ?? ('kategorie ' . $katId) }}</div>
            ...
            @foreach ($radky as $i => $r)
                <tr @class(['row-pending' => ! $r->approved])>
                    ...
                    <td class="mono font-bold">
                        {{ $r->callsign }}@if ($r->qrp)<x-badge variant="qrp" class="ml-1">QRP</x-badge>@elseif ($r->lp)<x-badge variant="lp" class="ml-1">LP</x-badge>@endif
                        {{-- Vizualizace vlastního deníku – jen pro řádek aktuálního závodníka s nahraným EDI --}}
                        @if ($e && $r->id === $e->id && $r->edi_head_id !== null)
                            <x-vizualizace-odkaz :head="$r->edi_head_id" target="_blank" class="ml-1" />
                        @endif
                    </td>
