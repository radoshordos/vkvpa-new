@props(['head', 'reduced' => false, 'target' => null])

{{-- Odkaz na EDI deník jako SVG ikona (dříve textové „EDI" / „EDIR").
     reduced = redukovaný deník oříznutý na závodní okno 08–11 UTC. --}}
@php
    $route = $reduced ? 'edi.soubor.redukovany' : 'edi.soubor';
    $title = $reduced ? 'Zobrazit redukovaný EDI deník (08–11 UTC)' : 'Zobrazit původní EDI soubor';
@endphp

<a href="{{ route($route, ['head' => $head]) }}"
   @if ($target) target="{{ $target }}" @endif
   {{ $attributes->merge(['class' => 'icon-btn icon-btn-ghost']) }}
   title="{{ $title }}"
   aria-label="{{ $title }}">
    @if ($reduced)
        {{-- Hodiny – redukce na závodní okno 08–11 UTC --}}
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            <circle cx="12" cy="12" r="9"/>
            <polyline points="12 7 12 12 16 14"/>
        </svg>
    @else
        {{-- Dokument – původní EDI soubor --}}
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
            <polyline points="14 2 14 8 20 8"/>
            <line x1="16" y1="13" x2="8" y2="13"/>
            <line x1="16" y1="17" x2="8" y2="17"/>
            <line x1="10" y1="9" x2="8" y2="9"/>
        </svg>
    @endif
</a>
